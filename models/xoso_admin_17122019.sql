...
-- ----------------------------
INSERT INTO `tbl_result` VALUES (4, '', '', '', '', '', '', '', '21', NULL, '', '2019-12-16 23:10:40', NULL, '2019-12-16 23:25:44', NULL, 'MB_TD', 1576429200);
...
-- ----------------------------
-- Procedure structure for sp_insertOrUpdateResult
-- ----------------------------
DROP PROCEDURE IF EXISTS `sp_insertOrUpdateResult`;
delimiter ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_insertOrUpdateResult`(IN `special` varchar(255),IN `first` varchar(255),IN `second` varchar(255),IN `third` varchar(255),IN `fourth` varchar(255),IN `fifth` varchar(255),IN `sixth` varchar(255),IN `seventh` varchar(255),IN `eighth` varchar(255),IN `symbol` varchar(255),IN `createdDate` datetime,IN `createdBy` varchar(255),IN `updatedDate` datetime,IN `updatedBy` varchar(255),IN `province` varchar(255),IN `date` BIGINT(20))
BEGIN
	IF (NOT EXISTS(SELECT * FROM xoso_admin.tbl_result as r WHERE r.province = province and r.date = date)) THEN
			INSERT INTO xoso_admin.tbl_result (special, first, second, third, fourth, fifth, sixth, seventh, eighth, symbol, 
			createdDate, createdBy, province, date) 
			VALUES(special, first, second, third, fourth, fifth, sixth, seventh, eighth, symbol, 
			createdDate, createdBy, province, date);
	ELSE
			UPDATE xoso_admin.tbl_result as r
			SET r.special = special, 
			r.first = first,
			r.second = second,
			r.third = third,
			r.fourth = fourth,
			...
			WHERE r.province = province and r.date = date;
	END IF;
